...
> この文書はMVP期の要件記録です。現行の動作は実装と関連テストを基準とし、READMEは現在の利用者向け入口、初回利用は[Getting Started](getting-started.md)を参照してください。State配置やFlowの詳細に差異がある場合は、現行実装を優先します。
...
v0.3.0の通過条件（gate）は、次の4種類の必須条件に限定します。
...
devflow は、v0.3.0では必須inputと必須artifactの通常ファイル存在、必須checkの現在entry sequenceでの成功、必須approvalの記録を確認します。
...
## v0.3.0: inputs と execution context

### `inputs`

stepは現在工程が受け取る成果物として `inputs` を持てる。`inputs` の要素はartifactと同じ `path` と `required` を持ち、`required` 省略時は `true` とする。既存Flowとの互換性のため、省略時は空配列として扱う。

required input は通常ファイルとして存在することを `done` のGate条件とする。input不足はartifact不足と区別し、`error_missing_required_input` で失敗する。input内、およびartifact内の同一pathは拒否するが、inputとartifactの間で同一pathを使うことは許可する。

### `devflow context`

`devflow context` は現在実行の安定したJSON契約をstdoutへ出力する読み取り専用コマンドとする。schema versionはState schema versionと独立した固定値 `1` とする。ファイル内容、AI呼び出し、runner、外部コマンド実行、freshness、fingerprint、check履歴は扱わない。

running StateではFlow run ID、Flow、status、entry sequence、step、inputs、artifacts、check状態、approval状態、completionを返す。completed / finished Stateでは同じヘッダ情報を返し、`step` と `completion` は `null` とする。Stateが存在しない、または不正な場合は既存CLIと同じくdiagnosticと非0終了を使い、疑似JSONは返さない。
