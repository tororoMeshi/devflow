...
v0.3.0の通過条件（gate）は、次の4種類です。

* 必須inputが通常ファイルとして存在すること
* 必須artifactが通常ファイルとして存在すること
* 必須checkが現在のentry sequenceで成功していること
* 承認が必要な工程で必須approvalが記録されていること
...
gateは、`step.inputs`、`step.artifacts`、`step.required_checks`、`step.approval` から devflow が導出します。`devflow done` はこれらの必須条件がすべて満たされた場合にのみ工程を進めます。
...
gateは、有効なFlow定義を受け取る前提で、実行時に必要なinputとartifactが実際に存在するかを確認します。
inputとartifactの `required` 省略値の解決も、Flow読み込みまたは正規化の責務とします。
...
* `step.inputs` を確認する
* projectRoot配下にrequired inputが通常ファイルとして存在するか確認する
* `step.artifacts` を確認する
* projectRoot配下にrequired artifactが通常ファイルとして存在するか確認する
* `step.required_checks` を確認し、現在のentry sequenceの成功結果を要求する
...
gateは `MissingInputs`、`MissingArtifacts`、`CheckProblems`、`MissingApprovals` を返します。
...
required input と required artifact は、projectRoot配下に通常ファイルとして存在する場合のみ満たされたものとします。
pathが存在してもディレクトリである場合は、不足として扱います。

`required: false` のinputとartifactは完了判定には使いません。
不足していてもgate OKとします。
`required` 省略時に `true` へそろえる処理はgateでは扱わず、Flow読み込みまたは正規化側のテストで確認します。

inputとartifactのテストでは、最低限次を確認します。

* required inputが存在する場合はOKになる
* required inputが存在しない場合は `MissingInputs` に入る
...
required check は、現在のentry sequenceに対応する記録があり、かつ `exit_code = 0` の場合だけ満たされたものとします。記録がない、または古いentry sequenceの結果は不足、非0の結果は失敗として `CheckProblems` に入れます。
...
input、artifact、check、approvalのいずれかが不足または失敗している場合も、最初の不足で止めず、すべてを `GateResult` に含めます。
...
state path は、`filepath.Join(ProjectRoot, ".devflow", "state.json")` とします。

flow dir は、`filepath.Join(ProjectRoot, ".devflow", "flows")` とします。

active Flow を必要とするcommandは、共通helperで `Store.Load()` を呼びます。

扱いは次の通りです。

* `LoadNoState` は active Flow なしとして非0で終了する
* `LoadOK` でも `status` が `completed` または `finished` の場合は active Flow なしとして非0で終了する
* `LoadInvalid` は非0で終了する
* `LoadOK` かつ `status == running` の場合のみ Flow定義を読み、`state.flow_id` と `current_step_id` の整合性を確認する

transition結果の保存条件は、次の通りです。

* `transitionResult.State != nil` の場合だけ `Store.Save()` する
* `transitionResult.State == nil` の場合は保存しない

出力先は、次の通りです。

* 通常出力、`status`、`prompt`、`list` の一覧は stdout に出す
* error diagnostic は stderr に出す
* warning diagnostic も stderr に出す
...
各CLIコマンドの処理を組み立てるための共通基盤を実装します。
...
internal/command/active_flow.go
internal/command/diagnostic.go
internal/command/*_test.go
```

完了条件:

* `Context` と `CommandResult` を定義する
* state path と flow dir を `ProjectRoot` から組み立てられる
* stdout / stderr を `io.Writer` 注入にできる
* active Flow を必要とするcommand向けの共通helperを用意する
* `Store.Load()` の `no_state` / `ok` / `invalid_state` を共通処理できる
* `state.flow_id` と `current_step_id` のFlow定義との整合性を確認できる
* `Diagnostic` を stdout / stderr 方針に従って最小限整形できる
* `transitionResult.State != nil` の場合だけ保存できる

### 10. init + initテンプレート

`devflow init` で作られる初期ディレクトリ、`.gitignore`、サンプルFlowを実装します。
...
* `.devflow/flows/` 配下のFlow定義を読む
* valid / invalid Flowを表示する
* 壊れたFlowがあってもvalidなFlowを捨てない
* 壊れたFlowが1つでもあれば非0で終了する
...
* `.devflow/flows/<flow>.cue` を読み、`state.json` を作成する
* active Flow が存在する場合は非0で終了する
* `completed` / `finished` のstateがある場合は新しいFlowを開始できる
* 失敗時に `state.json` を変更しない

### 13. status / prompt

現在状態表示とAI向け指示表示を実装します。
...
internal/command/status_test.go
internal/command/prompt_test.go
```

完了条件:

* `status` が active Flow の現在状態を表示する
* `prompt` が現在工程の指示、成果物、承認要否を表示する
* active Flow がない場合は非0で終了する
...
internal/command/approve_test.go
internal/command/done_test.go
```

完了条件:

* `approve` が現在工程または指定工程の承認を記録する
* `done` が gate 判定、transition、保存を接続する
* required artifact / approval 不足時は非0で終了し保存しない
* 成功時は `transitionResult.State != nil` の場合だけ保存する

### 15. back / skip / finish

戻る、スキップ、理由付き終了を実装します。
...
internal/command/back_test.go
internal/command/skip_test.go
internal/command/finish_test.go
```

完了条件:

* `back` が直前工程へ戻る
* `skip` が工程をスキップし、warning付き成功を `ExitCode = 0` として扱う
* `finish` が理由付きでFlowを終了する
* 失敗時に `state.json` を変更しない

### 16. cmd/devflow main.go
...
## v0.3.0: inputs と `devflow context`

### inputs

`step.inputs` は現在工程へ渡される成果物であり、`step.artifacts` は現在工程が作成する成果物である。両者は同じ `Artifact` モデルを使用する。Flow読み込み時にinputsが省略されていた場合は空sliceへ正規化し、artifactと同じpath安全性規則および `required: true` の既定値を適用する。

inputs内とartifacts内ではpath重複を拒否する。inputsとartifactsの間の同一pathは、入力を更新して同じpathへ出力する工程を許可するため有効とする。

Gateはrequired inputを通常ファイルとして確認する。input不足は `GateResult.MissingInputs` に保持し、`done` は `error_missing_required_input` を返してStateを更新しない。

### context JSON

`devflow context` はstdoutへJSONだけを出力する。Stateや成果物を変更せず、diagnosticはstderrへ出力する。schema versionは `1` であり、State schema versionとは独立する。

running Stateでは `gate.CheckDoneGate` を一度実行し、`completion.ready` をその `OK` と一致させる。blockerはinputs、artifacts、required checks、approvalの順に並べ、型は `missing_input`、`missing_artifact`、`missing_check`、`failed_check`、`missing_approval` とする。inputs / artifacts の `exists` とGateは同じ通常ファイル判定を共有する。completed / finishedでは `step` と `completion` を `null` にする。
